...
（安装 python3 环境后，将代码保存为 check_asset_reference.py ~~复制到 logseq 需整理图谱的根文件夹中~~，使用 cmd ~~在当前文件夹~~运行 `python3 c:\\cloudlet.info\\check_asset_reference.py` 即可将无效附件移动至自动建立的 to_delete 文件夹中）
